# Documentation

This folder contains the more technical project documentation that has been moved out of the top-level README.

## Start Here

- [setup.md](./setup.md): local setup and environment configuration
- [development.md](./development.md): development workflow, tests, and editor tooling
- [deployment.md](./deployment.md): CI, deployment, and production-facing notes
- [backups.md](./backups.md): automated backup configuration, testing, and restore notes
- [project-structure.md](./project-structure.md): architecture, stack, and repo layout
- [htmlisp-evolution-plan.md](./htmlisp-evolution-plan.md): proposed HTMLisp ergonomics and implementation plan
- [passkey-auth-plan.md](./passkey-auth-plan.md): passkey auth fit assessment and deferred implementation plan
- [performance-plan.md](./performance-plan.md): Lighthouse baseline and follow-up work
- [roadmap.md](./roadmap.md): future feature ideas and potential next steps
